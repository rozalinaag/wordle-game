document.addEventListener("DOMContentLoaded", () =>{
    ...
    function handleSubmitWord(){
        ...
        if (currentWord === word){
            window.alert("Congratulutaions!")
        }

        if (guesseWords.length === 6){
            window.alert(`Sorry, you have no more guesses! The word is ${word}`)
        }

        guesseWords.push([]); //чтобы переключиться на строчку ниже
    }
    ...
    function handleDeleteLetter(){
        ...
        guesseWords[guesseWords.length - 1] = currentWordArr;
